...
class BBoxUtility(object):
    ...
    def ssd_correct_boxes(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        ...
        #-----------------------------------------------------------------#
        # 3.27
        box_yx = box_xy[..., [1, 0]]  # 替代原来的::-1索引
        box_hw = box_wh[..., [1, 0]]
        
        input_shape = torch.tensor(input_shape, dtype=torch.float32, device=box_xy.device)
        image_shape = torch.tensor(image_shape, dtype=torch.float32, device=box_xy.device)
        if letterbox_image:
            # 使用PyTorch计算
    ...
    def decode_boxes(self, mbox_loc, anchors, variances):
        ...
        # 真实框的宽与高的求取
        decode_bbox_width   = torch.exp(mbox_loc[:, 2] * variances[1]) * anchor_width
        # decode_bbox_width   *= anchor_width
        decode_bbox_height  = torch.exp(mbox_loc[:, 3] * variances[1]) * anchor_height
        ...
        decode_bbox = torch.cat([
            decode_bbox_xmin.unsqueeze(-1),
            decode_bbox_ymin.unsqueeze(-1),
            decode_bbox_xmax.unsqueeze(-1),
            decode_bbox_ymax.unsqueeze(-1)
        ], dim=-1)
        
        decode_bbox = torch.clamp(decode_bbox, 0.0, 1.0)
        return decode_bbox
        # 真实框的左上角与右下角进行堆叠
    ...
    def decode_box(self, predictions, anchors, image_shape, input_shape, letterbox_image, variances = [0.1, 0.2], nms_iou = 0.3, confidence = 0.5):
        ...
        #---------------------------------------------------#
        #3.27
        anchors = anchors.to(predictions[0].device)
        ...
        for i in range(len(mbox_loc)):
            ...
            for c in range(1, self.num_classes):
                ...
                c_mask = c_confs > confidence
                
                if c_mask.sum() > 0:
                    # 保持数据在GPU
                    boxes_to_process = decode_bbox[c_mask]
                    confs_to_process = c_confs[c_mask]

                    keep = nms(boxes_to_process, confs_to_process, nms_iou)
                    
                    if keep.shape[0] > 0:
                        good_boxes = boxes_to_process[keep]
                        confs = confs_to_process[keep].unsqueeze(1)
                        # 自动匹配设备
                        labels = (c - 1) * torch.ones((len(keep), 1), device=confs.device)
                        
                        # 保持GPU计算直到最后
                        c_pred = torch.cat((good_boxes, labels, confs), dim=1)
                        batch_results.append(c_pred)

            if len(batch_results) > 0:
                batch_results = torch.cat(batch_results, dim=0)
                # GPU坐标修正
                box_xy = (batch_results[:, 0:2] + batch_results[:, 2:4]) / 2
                box_wh = batch_results[:, 2:4] - batch_results[:, 0:2]
                batch_results[:, :4] = self.ssd_correct_boxes(
                    box_xy, box_wh, 
                    input_shape, 
                    image_shape, 
                    letterbox_image
