...
if __name__ == "__main__":
    ...
    #   结果输出的文件夹，默认为map_out
    #-------------------------------------------------------#
    map_out_path    = 'fold5map'

    image_ids = open(os.path.join(VOCdevkit_path, "imageSets/test.txt")).read().strip().split()
    # image_ids = open(os.path.join(VOCdevkit_path, "imageSets/val_fold5.txt")).read().strip().split()
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            image_path  = os.path.join(VOCdevkit_path, "images/"+image_id)
        ...
        test_interval = 100 
        tact_time = ssd.get_FPS(image, test_interval)
        print(f"单次推理耗时: {tact_time * 1000:.2f} ms")
        print(f"FPS: {1 / tact_time:.2f}")
        ssd.get_model_info() 
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            ...
            with open(os.path.join(map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                root = ET.parse(os.path.join(VOCdevkit_path, "allxml/"+image_id+".xml")).getroot()
    ...
    if map_mode == 0 or map_mode == 3:
        ...
        print("Get vocmap done.")

    if map_mode == 0:
        print("Get cocomap.")
        get_coco_map(class_names = class_names, path = map_out_path)
        print("Get cocomap done.")
