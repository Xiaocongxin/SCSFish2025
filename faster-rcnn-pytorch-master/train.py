...
'''
class EarlyStopping:
    def __init__(self, patience=5, delta=0, path='best_weights.pth'):
        """
        Args:
            patience: 允许的验证损失不下降的epoch数
            delta:   视为改进的最小变化值
            path:    最佳模型保存路径
        """
    ...
    def __call__(self, val_loss, model):
        score = -val_loss  # 监控指标（这里用负损失，越大越好）
...
if __name__ == "__main__":
    ...
    #   预训练权重对于99%的情况都 必须要用，不用的话主干部分的权值太过随机，特征提取效果不明显，网络训练的结果也不会好
    #
    ...
    #   获得图片路径和标签
    #----------------------------------------------------#
    train_annotation_path   = 'train_fold5.txt'
    val_annotation_path     = 'val_fold5.txt'
    ...
    if True:
        ...
        early_stopper = EarlyStopping(
            patience=2,       # 连续10个epoch验证损失不下降则停止
            delta=0.001,       # 认为有改进的最小变化值
            path=os.path.join(save_dir, "best_early_stop.pth")
        )
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model, train_util, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir,early_stopper=early_stopper)
