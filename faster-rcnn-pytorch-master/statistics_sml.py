...
def img_size(file_name):
    ...
    img = cv2.imread(os.path.join('/D',dir_name, f'{img_name}.jpg'))  # 用于读取图片文件
    h, w = img.shape[:2]
    size = (w,h)
    return size
...
def process_files(dir_list, gt_dir, rord):
    count = [0,0,0,0]
    
    # 
    for dir_name in dir_list:
        dir_path = os.path.join('/S',dir_name,'detection-results') 
        print(dir_path)        
...
count1 = process_files(dirs_list, gt_dir, 0)
