...
def img_size(file_name):
    ...
    img = cv2.imread(os.path.join('images',dir_name, f'{img_name}.jpg'))  # 用于读取图片文件
    h, w = img.shape[:2]
    size = (w,h)
    return size
...
def process_files(dirs_list, gt_dir, class_names):
    ...
    class_all = {name: 0 for name in class_names}  

    for dir_name in dirs_list:
        detr_dir = os.path.join('faster-rcnn-pytorch-master',dir_name,'detection-results')
        print(detr_dir)
...
def parse_bboxes(file,size,num):
    ...
    for line in file.readlines(): 
        ...
        if num ==1:
            bbox = [float(part) for part in parts[1:5]]    # sd
            class_id = parts[0]
        if num ==0:
            conf = parts[1]
            if float(conf) <= 0.25:
                continue
            bbox = [float(part) for part in parts[2:6]]
            bbox= convert(size, bbox)
            class_name = parts[0]
            class_id = fish_species_dict[class_name]
            class_id = str(int(class_id))
        bboxes.append({'bbox': bbox, 'class_name': class_id})
    return bboxes

# dirs_list = ['fold1map','fold2map','fold3map','fold4map','fold5map']
dirs_list = ['fold1valmap','fold2valmap','fold3valmap','fold4valmap','fold5valmap']
gt_dir = 'labels'
class_name = [    
...
nanclass = ['acaja-a','acaja-m','acaja-z','acapy-a','acapy-m','acapy-z',
    'balun-a','balun-m','balun-z','bodax-a','bodax-m','bodax-z','cepar-a','cepar-m','cepar-z',
    'cepur-a','cepur-m','cepur-z','chaau-a','chaau-m','chaau-z','chakl-a','chakl-m','chakl-z',
    'chame-a','chame-m','chame-z','chrma-a','chrma-m','chrma-z','chrwe-a','chrwe-m','chrwe-z',
    'ctecy-a','ctecy-m','ctecy-z','ctest-a','ctest-m','ctest-z','dastr-a','dastr-m','dastr-z',
    'forfl-a','forfl-m','forfl-z','halho-a','halho-m','halho-z','hemme-a','hemme-m','hemme-z',
    'hench-a','hench-m','hench-z','labdi-a','labdi-m','labdi-z','melvi-a','melvi-m','melvi-z',
    'nasbr-a','nasbr-m','nasbr-z','nasli-a','nasli-m','nasli-z','parcr-a','parcr-m','parcr-z',
    'parmu-a','parmu-m','parmu-z','parpl-a','parpl-m','parpl-z','scobi-a','scobi-m','scobi-z',
    'shark-a','shark-m','shark-z','sufbu-a','sufbu-m','sufbu-z','zanco-a','zanco-m','zanco-z','fish-a','fish-m','fish-z']
