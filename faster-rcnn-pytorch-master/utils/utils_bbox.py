...
class DecodeBox():
    ...
    def frcnn_correct_boxes(self, box_xy, box_wh, input_shape, image_shape):
        ...
        #-------------------------------------------------------------#
        #   拼接成完整坐标格式 [x_min, y_min, x_max, y_max]
        #-------------------------------------------------------------#
        boxes = torch.cat([
            box_mins[..., 0:1],  # x_min
            box_mins[..., 1:2],  # y_min
        ...
        #-------------------------------------------------------------#
        #   将image_shape转换为与boxes相同设备和类型的张量
        #-------------------------------------------------------------#
        image_shape = torch.tensor(image_shape, dtype=torch.float32, device=boxes.device)
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, image_shape, input_shape, nms_iou=0.3, confidence=0.5):
        ...
        for i in range(bs):
            ...
            for c in range(1, self.num_classes):
                ...
                if c_mask.sum() > 0:
                    boxes_to_process = cls_bbox[c_mask, c]
                    ...
                    c_pred = torch.cat((good_boxes, confs, labels), dim=1)
            ...
            if len(batch_results) > 0:
                ...
                batch_results[:, :4] = self.frcnn_correct_boxes(box_xy, box_wh, input_shape, image_shape)
                # 转numpy
